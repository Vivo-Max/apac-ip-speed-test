...
def run_speed_test() -> str:
    ...
    try:
        ...
        # 实时读取和打印 stdout 和 stderr
        while process.poll() is None:
            # 读取 stdout
            stdout_line = process.stdout.readline()
            if stdout_line:
                stdout_line = stdout_line.strip()
                if stdout_line:  # 仅打印非空行
                    print(f"[iptest stdout] {stdout_line}")
                    logger.debug(f"[iptest stdout] {stdout_line}")

            # 读取 stderr
            stderr_line = process.stderr.readline()
            if stderr_line:
                stderr_line = stderr_line.strip()
                if stderr_line:  # 仅打印非空行
                    print(f"[iptest stderr] {stderr_line}")
                    logger.warning(f"[iptest stderr] {stderr_line}")
        ...
        if stdout:
            for line in stdout.splitlines():
                line = line.strip()
                if line:
                    print(f"[iptest stdout] {line}")
                    logger.debug(f"[iptest stdout] {line}")
        if stderr:
            for line in stderr.splitlines():
                line = line.strip()
                if line:
                    print(f"[iptest stderr] {line}")
                    logger.warning(f"[iptest stderr] {line}")
...
def main():
    ...
    try:
        if not args.generate_ips:
            ...
            try:
                logger.info(f"推送 {FINAL_CSV} 和 {IPS_FILE} 到远程仓库")
                ...
                if files_to_commit:
                    ...
                    subprocess.run(["git", "commit", "-m", "Update ip.csv and ips.txt with speed test results"], check=True)
                    subprocess.run(["git", "pull", "--rebase"], check=True)
                    subprocess.run(["git", "push"], check=True)
                    logger.info(f"成功推送 {', '.join(files_to_commit)}")
                else:
                    logger.info("无文件需要推送")
