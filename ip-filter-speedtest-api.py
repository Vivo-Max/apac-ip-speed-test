...
DESIRED_COUNTRIES = ['TW', 'JP', 'HK', 'SG', 'KR', 'IN', 'KP', 'VN', 'TH', 'MM', 'GB', 'US', 'DE']
...
def is_delay(s: str) -> bool:
    """验证延迟值"""
    return bool(re.match(r"^\d+(\.\d+)?\s*ms$", s)) or s.replace('.', '').isdigit()

def extract_ip_ports_from_file(file_path: str) -> List[Tuple[str, str, str]]:
    """从本地文件提取 IPv4 地址、端口和国家代码，只保留指定国家，按延迟排序"""
    ...
    header = rows[0]
    data_rows = rows[1:]
    logger.info(f"input.csv 总行数: {len(data_rows) + 1}")
    logger.info(f"CSV 字段数: {len(header)}, 示例行: {data_rows[0] if data_rows else '无数据'}")

    # 字段位置（根据 input.csv 结构调整）
    ip_col = 0      # IP 地址
    port_col = 1    # 端口
    country_col = 5 # 国家代码 (TW, JP 等)
    delay_col = 8   # 延迟

    logger.info(f"字段位置 - IP: {ip_col}, Port: {port_col}, Country: {country_col}, Delay: {delay_col}")
    ...
    for row in data_rows:
        ...
        try:
            ...
            ip_ports_with_delay.append((delay, ip, port, country))
    ...
    # 按延迟排序
    ip_ports_with_delay.sort(key=lambda x: x[0])
    ip_ports = [(ip, port, country) for _, ip, port, country in ip_ports_with_delay]
    logger.info(f"提取到 {len(ip_ports)} 个符合条件的节点")
    return ip_ports

def fetch_and_extract_ip_ports_from_url(url: str) -> List[Tuple[str, str, str]]:
    """从 URL 获取并提取 IPv4 地址、端口和国家代码，只保留指定国家，按延迟排序"""
    try:
    ...
    header = rows[0]
    data_rows = rows[1:]
    logger.info(f"URL CSV 总行数: {len(data_rows) + 1}")
    logger.info(f"CSV 字段数: {len(header)}, 示例行: {data_rows[0] if data_rows else '无数据'}")

    # 字段位置（根据 URL 返回的 CSV 结构调整）
    ip_col = 0      # IP 地址
    port_col = 1    # 端口
    country_col = 5 # 国家代码 (TW, JP 等)
    delay_col = 8   # 延迟
...
def filter_speed_and_deduplicate(csv_file: str):
    ...
    with open(csv_file, "r", encoding="utf-8") as f:
        ...
        try:
            header = next(reader)
            # 确保 ip.csv 的列为 IP,Port,DownloadSpeed(Mbps)，至少需要 3 列
            if len(header) < 3:
    ...
    with open(csv_file, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["IP", "Port", "DownloadSpeed(Mbps)"])  # 标准化表头
