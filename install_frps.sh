...
    # 删除原脚本中错误的 record_name 定义，直接使用 subdomain
    
...
            --data "{
                \"type\":\"A\",
                \"name\":\"$subdomain\", # <--- 修正点 1: 确保使用完整的子域名
                \"content\":\"$server_ip\",
                \"ttl\":120,
                \"proxied\":true
            }")
...
    case $1 in
        "status")
            # <--- 修正点 2: 确保将 systemctl 的输出通过管道/cat安全写入，避免权限问题
            sudo systemctl status "$2" --no-pager > /dev/null 2>&1 
            sudo systemctl status "$2" --no-pager 2>&1 | cat > $tmp_output
            ;;
        "log")
...
            ;;
        "restart_frps")
            # 将核心函数中的 log/warn 捕获到 tmp_output
            restart_frps_core 2>&1 | cat > $tmp_output
            ;;
        "stop_frps")
            # 将核心函数中的 log/warn 捕获到 tmp_output
            stop_frps_core 2>&1 | cat > $tmp_output
            ;;
        *)
            # 原始 Systemd 命令
            sudo systemctl "$1" "$2" 2>&1 | cat > $tmp_output
            ;;
    esac
    
    echo "----------------------------"
...
# --- 重启 frps 核心逻辑 ---
restart_frps_core(){
    # 使用 echo/log/warn 确保输出进入 execute_command 的管道
    echo "[INFO] 正在重启 frps..."
    if systemctl is-active frps >/dev/null 2>&1; then
        systemctl restart frps
        echo "[INFO] frps 已重启"
    else
        echo "[WARN] frps 未运行，将尝试启动..." >&2
        systemctl start frps
        if systemctl is-active frps >/dev/null 2>&1; then
            echo "[INFO] frps 已启动"
        else
            # 使用 err 会退出脚本，这里使用 warn 更合适，并提示查看日志
            echo "[ERROR] frps 启动失败，请检查日志 journalctl -u frps" >&2
        fi
    fi
}
...
    else
        TOKEN=$(grep -E '^(token|auth\.token|common\.token)' "$CONF_PATH" | awk -F'[ =]' '{print $NF}' | tr -d '"' | head -n 1 2>/dev/null || openssl rand -hex 16)
...
    TOKEN=$(grep -E '^(token|auth\.token|common\.token)' "$CONF_PATH" | awk -F'[ =]' '{print $NF}' | tr -d '"' | head -n 1 2>/dev/null)
    
    cat > "$CLIENT_TMPL" <<EOF
...
        TOKEN=$(grep -E '^(token|auth\.token|common\.token)' "$CONF_PATH" | awk -F'[ =]' '{print $NF}' | tr -d '"' | head -n 1 2>/dev/null || echo '未找到')
        echo "Token: $TOKEN"
        
