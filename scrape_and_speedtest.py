...
# 配置
URL = "https://raw.githubusercontent.com/gxiaobang2024/api/refs/heads/main/proxyip%20.csv"
IP_LIST_FILE = "ip.txt"
IPS_FILE = "ips.txt"
...
def check_dependencies() -> None:
    """检查依赖是否存在且可执行"""
    required_files = ['iptest', 'iptest.sh']
    for file in required_files:
        if not os.path.isfile(file):
            logger.error(f"依赖文件 {file} 不存在")
            sys.exit(1)
        if not os.access(file, os.X_OK):
            logger.error(f"依赖文件 {file} 没有执行权限")
            sys.exit(1)
        logger.info(f"依赖文件 {file} 检查通过")
...
def run_speed_test() -> str:
    """运行测速脚本，确保在 GitHub Actions 中稳定运行"""
    try:
        # 确保 iptest.sh 存在且可执行
        if not os.path.isfile(SPEEDTEST_SCRIPT):
            logger.error(f"测速脚本 {SPEEDTEST_SCRIPT} 不存在")
            return None
        if not os.access(SPEEDTEST_SCRIPT, os.X_OK):
            logger.error(f"测速脚本 {SPEEDTEST_SCRIPT} 没有执行权限")
            return None
        ...
        if return_code == 0 and os.path.exists(FINAL_CSV):
            ...
            logger.error(f"测速失败或未生成 {FINAL_CSV}: 返回码 {return_code}")
            return None
    except Exception as e:
